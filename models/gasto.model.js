...
const create = async({ roommate_id, comment, amount})=>{
    try {
        const gastosFile = await readFile(path.join(__dirname, '../data/gastos.json'));
        const gastosData = JSON.parse(gastosFile);

        const roommatefile = await readFile(path.join(__dirname, '../data/roommates.json'));
        ...
        await writeFile(path.join(__dirname, '../data/roommates.json'), JSON.stringify(roommateData));
    ...
    try {
        ...
        roommateData.forEach(e => {
            if(e.id  === gastosData[index].roommate_id){
                Object.assign(e, {debit: parseInt(e.debit)-parseInt(gastosData[index].amount)})
            }
        });
